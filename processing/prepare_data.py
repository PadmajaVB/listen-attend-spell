...
def prepare_data(datadir, featdir, conf, feat_type, dynamic):
    ...
    for utt in wavfiles:
        if found_segments:
            ...
            # print "Rate utt = ", rate_utt[utt][1], "type = ", type(rate_utt[utt][1])
...
def read_wav(wavfile):
    ...
    Args:
        wavfile: a pair containing either the filename or the command to read the
    ...
    if wavfile[1]:
        ...
        os.system(wavfile[0] + ' tee tmp.flac > duplicate.flac')
        # read the created wav file
        (utterance, rate) = sf.read('tmp.flac')
        # delete the create file
        os.remove('tmp.flac')
        os.remove('duplicate.flac')
    else:
        (utterance, rate) = sf.read(wavfile[0])
